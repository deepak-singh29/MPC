 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
	...
	for (int t = 1; t < N; t++) {
	  ...
	  // considering actuation at time t
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // Set the initial state values in vars
  vars[x_start] = x;
  vars[y_start] = y;
  vars[psi_start] = psi;
  vars[v_start] = v;
  ...
  // Setting all non-actuator parameters to -infinity to +infinity
  for(size_t i = 0;i<delta_start;i++){
	  vars_lowerbound[i] = -1.0e19;
	  vars_upperbound[i] = 1.0e19;
  }
  // Setting limits for delta (in radians) between -25 and 25 degrees
  for(size_t i = delta_start ; i<a_start;i++){
	  vars_lowerbound[i] = -0.436332;
	  vars_upperbound[i] = 0.436332;
  }
  // setting acceleartion/decceleartion limits
  ...
  for (size_t i = 0; i < n_constraints; i++) {
   ...
   // Constartints for initial state
